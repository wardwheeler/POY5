...
let set_up_params in_array =
  ...
  for i = 0 to Array.length in_array - 1 do
      match in_array.(i) with 
        ...
        (*| "-sample_size" -> l_sample_size := int_of_string in_array.(i + 1);
        | "-t_m_mean" -> l_t_m_mean := float_of_string in_array.(i + 1);
        | "-t_m_var" -> l_t_m_var := float_of_string in_array.(i + 1);
        | "-ns" -> l_ns := int_of_string in_array.(i + 1);
        | "-nv" -> l_nv := float_of_string in_array.(i + 1);
        | "-t_gen_mean" -> l_t_gen_mean := float_of_string in_array.(i + 1);
        | "-t_gen_var" -> l_t_gen_var := float_of_string in_array.(i + 1);
        | "-uniform_min" -> l_uniform_min := float_of_string in_array.(i + 1);
        | "-uniform_max" -> l_uniform_max := float_of_string in_array.(i + 1);*)
        | _ -> if ((i mod 2) = 1) then Printf.printf "Unrecognized option %s\n" in_array.(i);
  done;
