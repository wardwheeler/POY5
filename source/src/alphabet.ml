...
let of_file fn orientation init3D level respect_case tie_breaker alphabet =
    ...
    let alph = match alphabet with
      | None ->
          ...
          of_string ~respect_case ~orientation ~init3D elts default_gap None
      | Some x -> create_alph_by_level x level
    in
    ...
    * combination, size set to original size, level set to 0, two maps set to empty, *)
    ...
    if alph.gap != (Cost_matrix.Two_D.gap tcm_full)
      then failwithf "Expected alphabet gap (%d) is not in the full cost-matrix with %d columns" alph.gap (Cost_matrix.Two_D.gap tcm_full);
    if alph.gap != Cost_matrix.Two_D.gap tcm_original
      then failwithf "Expected alphabet gap (%d) is not in the orig cost-matrix with %d columns" alph.gap (Cost_matrix.Two_D.gap tcm_original);
